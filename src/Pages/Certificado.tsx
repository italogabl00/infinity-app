...
function Alunos() {
  ...
  const [alunoEncontrado, setAlunoEncontrado] = useState<any | null>(null);
  ...
  const buscarAluno = () => {
    ...
    let aluno = alunos.find((aluno: any) => aluno.token === searchQuery);

    // Se não encontrar pelo token, tenta buscar pelo nome
    if (!aluno) {
      aluno = alunos.find(
        (aluno: any) => aluno.nome.toLowerCase() === searchQuery.toLowerCase()
      );
    }
